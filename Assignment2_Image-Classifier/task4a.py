...
def convolve_im(im: np.array,
    ...
    # Translate image to Frequency domain. Kernel should already be in FD.
    fft_im = np.fft.fft2(im)

    # Do the multiplication.
    convolved_fft_im = fft_im * fft_kernel
    # Shift back to spatial domain.
    convolved_sp_im = np.fft.ifft2(convolved_fft_im)
    conv_result = np.real(convolved_sp_im)
